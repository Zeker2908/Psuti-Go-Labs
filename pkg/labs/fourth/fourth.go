...
// PrintPeople выводит все записи на экран.
func PrintPeople(mapped map[string]int) {
	if mapped == nil {
		fmt.Println("Ошибка: длины списков имен и возрастов не совпадают.")
		return
	}
...
// AddPerson добавляет нового человека в карту.
func AddPerson(mapped map[string]int, name string, age int) {
	mapped[name] = age
}
